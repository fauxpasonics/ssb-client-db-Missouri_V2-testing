...
CREATE PROC [rpt].[Basketball_HOB_3] (@SeasonYear INT)
AS

--DECLARE @SeasonYear INT = 2018
--EXEC rpt.Basketball_HOB_3 @SeasonYear
...
FROM (
		SELECT DimEvent.EventDate
			  ...
			  ,0 BudgetQTY
			  ,0 BudgetRev
			  ,COUNT(*) OVER() AS EventsInSeason
		FROM dbo.DimEvent DimEvent
			JOIN dbo.DimSeason DimSeason ON DimSeason.DimSeasonId = DimEvent.DimSeasonId
		WHERE SeasonClass = 'Men''s Basketball'
...
SELECT dimEvent.EventCode
	  ,dimEvent.EventDate
	  ,CASE WHEN dimTicketType.TicketTypeName = 'group' THEN 'Group' ELSE 'Public' END TicketType
	  ,SUM(fts.QtySeat) QTY
...
WHERE 1=1
	  ...
	  AND SeasonClass = 'Men''s Basketball'
	  AND seasonyear = @SeasonYear
