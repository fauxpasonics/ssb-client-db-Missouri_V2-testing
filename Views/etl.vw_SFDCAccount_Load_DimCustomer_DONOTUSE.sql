...
CREATE VIEW [etl].[vw_SFDCAccount_Load_DimCustomer_DONOTUSE] AS (
...
*/
		...
		SELECT
			...
			,'Mizzou PC_SFDC Account' SourceSystem
			...
			/*AddressPrimary*/
			, BillingStreet [AddressPrimaryStreet]
			, BillingCity [AddressPrimaryCity] 
			, BillingState [AddressPrimaryState] 
			, BillingPostalCode [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, [BillingCountry] [AddressPrimaryCountry] 
			...
			, CASE WHEN (SELECT value FROM [dbo].[fn_Split](p.SSB_CRMSystemArchticsIDs__c,', ') WHERE position = 1) IS NOT NULL
				...
				END AccountID
			, 1 IsBusiness --CASE WHEN z.id IS NOT NULL THEN 1 ELSE 0 END AS IsBusiness
...
--	select top 100 *
	FROM  Missouri_Reporting.ProdCopy.Account p
	WHERE 1=1
	--AND p.copyloaddate > DATEADD(DAY,-5,GETDATE())
	AND RecordTypeId <> '0126A000000cui9QAA' --This changes from client to client. Confirmed for Missouri (Business Account)
